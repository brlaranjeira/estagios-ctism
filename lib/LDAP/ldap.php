...
class ldap {
    ...
    public function getXbyY($x, $y, $value, $dn = LDAP_PEOPLE_BASE, $logOp = 'or') {
    	$logOp = strtolower($logOp) == 'or' ? '|' : '&';
        ...
        if (sizeof($fields) > 1) {
			...
			while ( $entry ) {
				...
				for ($i=0; $i < $size_i; $i++) {
					$attr_name = $attrs[$i];
					$curr_attr = $attrs[$attr_name];
					if ($curr_attr['count'] == 1) {
						$current[$attr_name] = $curr_attr[0];
					} else {
						foreach ( $curr_attr[$attr_name] as $tmp ) {
							$current[$attr_name][] = $tmp;
						}
					}
				}
				$array[] = $current;
    ...
    public function isMembroDoGrupo($usuario, $grupo) {
        ...
        $users = $this->getXbyY('memberuid', $by, $grupo, LDAP_GROUPS_BASE);
        return (@in_array($usuario, $users) ? true : false);
    }
